...
if ! kubectl get secret cloudflare-api-token -n cert-manager &>/dev/null; then
  if [[ -z "${CLOUDFLARE_API_TOKEN:-}" ]]; then
    echo "  CLOUDFLARE_API_TOKEN not set — skipping LE issuers (internal CA will still work)"
    echo "  Re-run with: CLOUDFLARE_API_TOKEN=<token> $0"
  else
...
kubectl wait certificate rdm-ca \
  --namespace cert-manager \
  --for=condition=Ready \
  --timeout=60s
...
echo "    kubectl get secret rdm-ca-secret -n cert-manager -o jsonpath='{.data.tls\\.crt}' | base64 -d > rdm-ca.crt"
echo "    # Then add rdm-ca.crt to your OS/browser trust store"
